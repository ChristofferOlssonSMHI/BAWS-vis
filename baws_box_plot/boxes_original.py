...
if __name__ == "__main__":
    stats_df = pd.read_excel(
        r'C:\Utveckling\BAWS-vis\bawsvis\examples\area_season_bloom_all.xlsx',
    ...
    for i, (p, df_set) in enumerate(zip(bar_plots, data_bloom)):
        for (index, row), patch in zip(df_set.iterrows(), p.patches):
            ...
            if i == len(data_bloom) - 1:
                ...
                _x = stats_df['std_dev_end_hi'].max() + 1
    ...
    ax.set_xticklabels(['', 'Juni', '', '', 'Juli', '', '', 'Augusti', ''], minor=True)

    plt.tight_layout()
    plt.show()
    plt.savefig(f'area_season_bloom_diagram_with_clouds_80percent_{YEAR}.png', dpi=600)
